...
ACMD_FUNC(autohunt){
	...
	if (strcmp(subcmd, "start") == 0) {
		...
		if (strlen(rest) == 0) {
			...
			char msg[512];
			snprintf(msg, sizeof(msg),
				"[Auto-Hunt] Config | Skill: %d/%d | HP: %d%% | SP: %d%% | Range: %d | Potion: %s (%d) | Loot: %s",
				cfg->skill_id, cfg->skill_level,
				cfg->hp_threshold, cfg->sp_threshold,
				cfg->target_range,
				cfg->potion_id > 0 ? "ON" : "OFF", cfg->potion_id,
				cfg->loot_enabled ? "ON" : "OFF");
			clif_displaymessage(fd, msg);
		...
		if (strcmp(setting, "skill") == 0) {
			...
			if (value1 == 0 && strcmp(extra, "auto") == 0) {
				...
				if (!ahd2) {
					autohunt.getConfig(sd->id);
					ahd2 = autohunt.getData(sd->id);
				}
				if (ahd2) {
					autohunt.autoDetectSkill(sd, ahd2);
				}
				...
				cfg->skill_id = (uint16)value1;
				cfg->skill_level = (uint16)value2;
				char msg[256];
				snprintf(msg, sizeof(msg), "[Auto-Hunt] Skill set to %d level %d", value1, value2);
				clif_displaymessage(fd, msg);
			} else {
				clif_displaymessage(fd, "Usage: @autohunt config skill [auto|<id> <lv>|0]");
				clif_displaymessage(fd, "  auto     - Auto-detect from hotbar");
				clif_displaymessage(fd, "  0        - Normal attack only");
				clif_displaymessage(fd, "  <id> <lv> - Set specific skill");
			}
		} else if (strcmp(setting, "hp") == 0) {
			cfg->hp_threshold = (int16)value1;
			char msg[256];
			snprintf(msg, sizeof(msg), "[Auto-Hunt] HP threshold set to %d%%", value1);
			clif_displaymessage(fd, msg);
		} else if (strcmp(setting, "sp") == 0) {
			cfg->sp_threshold = (int16)value1;
			char msg[256];
			snprintf(msg, sizeof(msg), "[Auto-Hunt] SP threshold set to %d%%", value1);
			clif_displaymessage(fd, msg);
		} else if (strcmp(setting, "range") == 0) {
			if (value1 > 0 && value1 <= 150) {
				cfg->target_range = (int16)value1;
				char msg[256];
				snprintf(msg, sizeof(msg), "[Auto-Hunt] Range set to %d", value1);
				clif_displaymessage(fd, msg);
			} else {
			...
			if (value1 == 0 && extra[0] == '\0') {
				cfg->use_potion = false;
				cfg->potion_id = 0;
				clif_displaymessage(fd, "[Auto-Hunt] Auto-potion disabled.");
			} else if (value1 > 0) {
				cfg->use_potion = true;
				cfg->potion_id = (t_itemid)value1;
				char msg[256];
				snprintf(msg, sizeof(msg), "[Auto-Hunt] Auto-potion set to item %d", value1);
				clif_displaymessage(fd, msg);
			} else {
				clif_displaymessage(fd, "Usage: @autohunt config potion <item_id|0>");
				clif_displaymessage(fd, "  Red Potion=501, White=504, White Slim=547");
				clif_displaymessage(fd, "  0 = Disable auto-potion");
			}
		} else if (strcmp(setting, "loot") == 0) {
			char extra[64] = "";
			sscanf(rest, "%*s %63s", extra);
			if (value1 == 0 && (strcmp(extra, "off") == 0 || extra[0] == '\0')) {
				cfg->loot_enabled = false;
				clif_displaymessage(fd, "[Auto-Hunt] Auto-looting disabled.");
			} else if (value1 == 0 && strcmp(extra, "on") == 0) {
				cfg->loot_enabled = true;
				clif_displaymessage(fd, "[Auto-Hunt] Auto-looting enabled.");
			} else {
				clif_displaymessage(fd, "Usage: @autohunt config loot <on|off>");
			}
		} else {
			clif_displaymessage(fd, "Usage: @autohunt config [skill/hp/sp/range/potion/loot] <value>");
			clif_displaymessage(fd, "  skill <id> <lv> - Set skill to use");
			clif_displaymessage(fd, "  hp <percent>   - HP stop threshold");
			clif_displaymessage(fd, "  sp <percent>   - SP stop threshold");
			clif_displaymessage(fd, "  range <1-50>   - Search range");
			clif_displaymessage(fd, "  potion <id>    - Auto-potion item ID (0=off)");
			clif_displaymessage(fd, "  loot <on|off>  - Auto-looting toggle");
		}
	} else {
		clif_displaymessage(fd, "Usage: @autohunt [start|stop|status|config]");
	}
